...
function useGatherResources(quizID, members, username) {
  ...
  return [memberResults, quiz, error, loading, userResult];
}
...
function getUserResult(userResult) {
  if (userResult === null) {
    ...
    return <p>{userResult.result}</p>;
  }
}

function StatsBox({ group, quizID, result, username }) {
  const [memberResults, quiz, error, loading, userResult] = useGatherResources(
    quizID,
    group.members.items,
    username
  );
